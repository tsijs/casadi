...
namespace casadi {
  ...
  = {{&Nlpsol::options_},
     {{"iteration_callback",
      ...
      {"fg",
       {OT_FUNCTION,
        "Function for calculating the gradient of the objective and the constraints "
        "(overrules the nlpsol init)"}},      
     }
  };
  ...
  void Blocksqp::init(const Dict& opts) {
    ...
    // skip this when handing manual functions for now. 
    if (restore_feas_ && !has_function("nlp_fg")) {
      // get orignal nlp
    ...
    } else if (has_function("nlp_fg") && restore_feas_) {
      std::cout << "restauration options ignored because manual optimization functions are passed. Not yet implemented and will take the wrong dummy functions to optimize if used now."<< std::endl;
    }
    ...
    if (!has_function("nlp_gf_jg"))
      ...
      gf_jg = get_function("nlp_gf_jg");

    Asp_ = gf_jg.sparsity_out(3);//"jac_g_x" TODO give the callbacks names? For now use default name as workaround
  ...
  void Blocksqp::calcInitialHessian(BlocksqpMemory* m) const {
    // if (false && has_function("nlp_hess_approx_init")) {
  ...
  solveQP(BlocksqpMemory* m, double* deltaXi, double* lambdaQP,
    ...
    opts.initialStatusBounds = qpOASES::ST_INACTIVE; //TODO experiment with this
    opts.printLevel = qpOASES::PL_NONE;// TODO make this an option
    ...
    for (l=0; l<maxQP; l++) {
        ...
        if (matricesChanged) {
          ...
          // TODO add sparsity of exact part to this 
          convertHessian(m);
          delete m->H;
          m->H = nullptr;
