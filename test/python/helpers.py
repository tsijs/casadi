...
class casadiTestCase(unittest.TestCase):
  ...
  def check_codegen(self,F,inputs=None, opts=None,std="c89",extralibs="",check_serialize=False):
    if args.run_slow:
      ...
      commands = "gcc -pedantic -std={std} -fPIC -shared -Wall -Werror -Wextra -Wno-unused-variable -I{includedir} -Wno-unknown-pragmas -Wno-long-long -Wno-unused-parameter -O3 {name}.c -o {name}.so -L{libdir}".format(std=std,name=name,libdir=libdir,includedir=includedir) + extralibs
